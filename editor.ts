...
a function buildDecorations(view: any, plugin: any) {
    ...
    for (const { from, to } of view.visibleRanges) {
        ...
        while ((match = REF_PATTERN.exec(text)) !== null) {
            ...
            allDecos.push(
                { from: blockStart, to: innerStart, deco: Decoration.replace({}) },
                { from: innerEnd, to: blockEnd, deco: Decoration.replace({}) },
            );
            decorated.push(
                { from: blockStart, to: innerStart },
                { from: innerEnd, to: blockEnd },
            );

            const cleanMatch = match[0].replace(/\*\*/g, '').replace(/\*/g, '');
            const parsed = plugin.safeParse?.(
                cleanMatch.replace('{{', '⟪⟪').replace('}}', '⟫⟫'),
            );
            if (!parsed) continue;
        ...
        if (plugin.settings.autoDetect) {
            const ranges = [...decorated].sort((a, b) => a.from - b.from);
            let pos = from;
